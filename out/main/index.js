...
const logger$c = createLogger("ConfigService");
class ConfigService {
  store;
  defaultSettings = {
    llmProvider: "openai",
  ...
  async initialize() {
    logger$c.info("ConfigService initialized");
  }
  async get() {
  ...
  async set(settings) {
    ...
    logger$c.info("Settings updated");
  }
  async testApiKey(provider, apiKey) {
    logger$c.info(`Testing API key for provider: ${provider}：${apiKey}`);
    return true;
  }
  async cleanup() {
    logger$c.info("ConfigService cleaned up");
  }
}
const setExecutablePermission = async (filePath) => {
  await execa.execa("chmod", ["+x", filePath]);
};
...
const logger$b = createLogger("HttpClient");
class HttpClient {
  client;
  baseURL;
  constructor(config) {
  ...
  setupInterceptors() {
    this.client.interceptors.request.use(
      (config) => {
        logger$b.debug(`Making ${config.method?.toUpperCase()} request to ${config.url}`);
        return config;
      },
      (error) => {
        logger$b.error("Request interceptor error:", error);
        return Promise.reject(error);
      }
    );
    this.client.interceptors.response.use(
      (response) => {
        logger$b.debug(`Response received: ${response.status} ${response.statusText}`);
        return response;
      },
      (error) => {
        if (error.response) {
          logger$b.error(`HTTP Error ${error.response.status}: ${error.response.statusText}`);
          logger$b.debug("Error response data:", error.response.data);
        } else if (error.request) {
          if (error.code === "ECONNREFUSED") {
            logger$b.debug("Connection refused - service may not be running:", error.message);
          } else {
            logger$b.error("No response received:", error.message);
          }
        } else {
          logger$b.error("Request setup error:", error.message);
        }
        return Promise.reject(error);
      }
    );
  ...
  async request(config, retries = 0) {
    try {
      ...
      if (retries < maxRetries && this.shouldRetry(error)) {
        logger$b.warn(`Request failed, retrying in ${retryDelay}ms... (${retries + 1}/${maxRetries})`);
  ...
  shouldRetry(error) {
    ...
    return !error.response || error.code === "ETIMEDOUT" || error.code === "ENOTFOUND" || error.code === "ECONNRESET" || error.response && error.response.status >= 500 && error.response.status < 600;
  }
  /**
   * 延迟函数
   */
  ...
  async healthCheck(endpoint = "/health") {
    try {
      await this.get(endpoint, { timeout: 3e3 });
      return true;
    } catch (error) {
      logger$b.debug("Health check failed:", error);
      return false;
    }
  }
  /**
  ...
  updateBaseURL(baseURL) {
    ...
    logger$b.info(`Base URL updated to: ${baseURL}`);
  }
  /**
   * 获取当前基础URL
   */
...
const logger$a = createLogger("ChatlogHttpClient");
class ChatlogHttpClient {
  httpClient;
  defaultRetries = 3;
  ...
  constructor(baseUrl = "http://127.0.0.1:5030") {
    ...
    logger$a.info(`ChatlogHttpClient initialized with base URL: ${baseUrl}`);
  }
  /**
   * 检查 Chatlog 服务状态
   */
  async checkServiceStatus() {
    try {
      ...
      if (error.code === "ECONNREFUSED") {
        logger$a.debug("Chatlog service is not running on", this.httpClient.getBaseURL());
      } else {
        logger$a.debug("Chatlog service health check failed:", error.message);
      }
      return false;
    }
  }
  ...
  async getContacts(params) {
    try {
      ...
      logger$a.error("Failed to get contacts:", error);
  ...
  async getMessages(params) {
    try {
      ...
      logger$a.error("Failed to get messages:", error);
  ...
  async getChatroomInfo(chatroomId) {
    try {
      const data = await this.httpClient.get(`/api/v1/chatroom/${chatroomId}`, {
        retries: this.defaultRetries,
        retryDelay: this.defaultRetryDelay
      });
      return {
      ...
      logger$a.error(`Failed to get chatroom info for ${chatroomId}:`, error);
      return null;
    }
  }
  /**
  ...
  async getSessions() {
    try {
      const data = await this.httpClient.get("/api/v1/session", {
        retries: this.defaultRetries,
        retryDelay: this.defaultRetryDelay
      });
      ...
      logger$a.error("Failed to get sessions:", error);
  ...
  updateBaseUrl(baseUrl) {
    ...
    logger$a.info(`Chatlog service URL updated to: ${baseUrl}`);
  }
  /**
   * 获取当前服务地址
   */
...
class ChatlogService {
  ...
  getChatlogWorkDir() {
    ...
    const workDir = savedWorkDir || path__namespace.join(homeDir, "Documents", "EchoSoul", "chatlog_data");
    if (!fs__namespace.existsSync(workDir)) {
      fs__namespace.mkdirSync(workDir, { recursive: true });
      logger$9.info(`Created chatlog data directory: ${workDir}`);
    }
    return workDir;
  }
  /**
  ...
  setWorkDir(workDir) {
    ...
    logger$9.info(`Work directory set to: ${workDir}`);
  }
  /**
   * 获取当前工作目录
   */
   ...
   * 安全地检查服务状态，不会抛出异常
   */
  async checkStatusSafely() {
    try {
      return await this.checkStatus();
    } catch (error) {
      logger$9.debug("Safe status check failed:", error);
      return "not-running";
    }
  }
  async startService() {
   ...
   * 检查是否已有解密后的数据
   */
  async checkDecryptedData(workDir) {
    try {
      if (!fs__namespace.existsSync(workDir)) {
        return false;
      }
      const findDbFiles = (dir) => {
        ...
        try {
          ...
          logger$9.debug(`Error reading directory ${dir}:`, error);
        }
        return files;
      };
      ...
      logger$9.debug("Error checking decrypted data:", error);
      return false;
    }
  }
  /**
  ...
  async checkInitialization() {
    try {
      ...
      logger$9.debug("Error checking initialization:", error);
      return {
  ...
  async getChatroomInfo(chatroomId) {
    try {
      ...
      logger$9.error(`Failed to get chatroom info for ${chatroomId}:`, error);
      return null;
    }
  }
  /**
  ...
  async getSessions() {
    try {
      ...
      logger$9.error("Failed to get sessions:", error);
      return [];
    }
  }
  /**
  ...
  updateServiceUrl(newUrl) {
    ...
    logger$9.info(`Chatlog service URL updated to: ${newUrl}`);
  }
  async cleanup() {
...
`;
  ...
  async saveReport(content, type2, config) {
    ...
    const reportMeta = {
      ...
      type: type2,
      date,
      title: type2 === "auto" ? `${date} 每日报告` : `自定义报告 - ${date}`,
      filePath,
      metadata: {
        messageCount: 0,
        // TODO: 从实际数据获取
...
function registerInitializationHandlers() {
      ...
      if (!initializationManager) {
        ...
        initializationManager.on("stateChanged", (state) => {
          electron.BrowserWindow.getAllWindows().forEach((window2) => {
            window2.webContents.send("initialization:stateChanged", state);
          });
        });
        initializationManager.on("completed", () => {
          electron.BrowserWindow.getAllWindows().forEach((window2) => {
            window2.webContents.send("initialization:completed");
          });
        });
        initializationManager.on("error", (error) => {
          electron.BrowserWindow.getAllWindows().forEach((window2) => {
            window2.webContents.send("initialization:error", error);
          });
        });
      }
  ...
  electron.ipcMain.handle("initialization:getDiagnostics", async () => {
    try {
      const { ChatlogDiagnostics } = await Promise.resolve().then(() => require("./ChatlogDiagnostics-5F9nmaAJ.js"));
...
electron.app.whenReady().then(async () => {
  utils$2.electronApp.setAppUserModelId("com.echosoul");
...
exports.ChatlogHttpClient = ChatlogHttpClient;
