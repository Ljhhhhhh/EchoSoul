...
import type { ChatMessage, AnalysisConfig, BasicStats } from '@echosoul/common';
...
export class AnalysisService {
  ...
  private calculateBasicStats(messages: ChatMessage[]): BasicStats {
    ...
    const sendReceiveRatio =
      totalMessages > 0 ? sentMessages / totalMessages : 0;
    ...
    messages.forEach(message => {
      const contact = message.isGroupChat
  ...
  private async analyzeMessages(messages: ChatMessage[], stats: BasicStats) {
    ...
    return {
      ...
      summary:
        '您是一个积极乐观的人，善于表达，在社交中比较活跃，主要关注工作和生活话题。',
    };
  }
