...
export class InitializationManager extends EventEmitter {
  ...
  private async selectWorkDir(): Promise<InitializationResult> {
    ...
    // 发送事件让前端显示目录选择对话框
    this.updateStepStatus(InitializationStep.SELECTING_WORKDIR, InitializationStatus.WAITING_USER_INPUT);
  ...
  private async tryStartService(): Promise<boolean> {
    try {
      const status = await this.chatlogService.checkStatusSafely();
      if (status === 'running') {
        return true;
      }
